...
@WebServlet("/consulCheckNQuarto")
public class consulCheckNQuarto extends HttpServlet {
	private static final long serialVersionUID = 1L;
    
    public consulCheckNQuarto() {
        super();
    }
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			st = conn.prepareStatement("SELECT id_reserva FROM hotel.tb_reserva; ");
			rs = st.executeQuery();	 
				while(rs.next()) {
			    ...
			    out.println("<title>CONSULTA DAS RESERVAS</title>");
			    out.println("<style>");
			    ...
			    if(sele.size() != 0){
			    	int ie = 0;
			    		...
			    		out.print("<h1>Reservas realizadas: </h1>");
			    		for (int i1 = 0; i1 <= sele.size() - 1; i1++) {
							   out.println("ID da Reserva: " + sele.get(i1) + "<br>"); 
							}
			    	...
			    	response.sendRedirect("HospN2.jsp");
			    }
			    out.println(" <p><a style='color: black;' href=\"consCheck.jsp\">CLIQUE AQUI PARA VOLTAR</a></p>");
			    out.println("</form>");
			    out.println("</div>");
